...
class Joint {
    constructor (x, y, a=0, parent=null) {
        this.x = x;
        this.y = y;
        this.a = a;
        this.parent = parent;
    ...
    _get(t) {
        ...
        const x = Math.cos(p.a) * tx - Math.sin(p.a) * ty;
        const y = Math.sin(p.a) * tx + Math.cos(p.a) * ty;
...
class Face {
    constructor(head, joint=null) {
        this.joint = joint || head.joint;
        let eyeCount = randInt(0, 2) + randInt(0, 2) + randInt(0, 2) + randInt(0, 2);
        if(chance(0.7)) eyeCount = 2;
        this.eyes = []
        let tryCount = 50;
        let yMin = Math.round(3-head.size);
        let xMax = Math.round(head.size/2);
        while(tryCount && eyeCount > this.eyes.length){
            tryCount--;
            if(eyeCount - this.eyes.length == 1 || chance(0.3)){
                let y = randInt(yMin, 0);
                let s = chance(0.4 + (eyeCount**0.6)/10) ? 1 : chance(0.6)? 1.25 : 1.6;
                ...
                let x = randInt(3, xMax);
                let y = randInt(yMin, 0);
                let s = chance(0.5 + (eyeCount**0.6)/10) ? 1 : 1.25;
...
class Tail {
    constructor(tailJoint, jointCount=10) {
    ...
    async draw(grid, t) {
        const spline = this.spline.get(t);
...
class Neck {
    constructor(neckJoint, headJoint, width, jointCount=2) {
        this.width = width;
...
class Leg {
    constructor(hip, length, jointCount, width, footXOffset=0){
        this.width = width;
